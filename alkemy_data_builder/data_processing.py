...
def data_processing(list_files: Optional[List[Path]]) -> pd.DataFrame:
    ...
    # para el logging
    logger.info(
        f"Se genero un unico DataFrame con toda la informacion normalizada de las tres fuentes"
    )
    return final_data


def cleaning_provinces(data: Optional[pd.DataFrame]) -> pd.DataFrame:
    """A partir de observar los datos, se detectaron algunos registros con diferencias en la carga de las provicias y un
    ...
    """
    data.provincia = data.provincia.apply(
        lambda x: str(x).replace(
            "Tierra del Fuego, Antártida e Islas del Atlántico Sur", "Tierra del Fuego"
        )
    )
    data.provincia = data.provincia.apply(
        lambda x: str(x).replace("Santa Fé", "Santa Fe")
    )
    data.provincia = data.provincia.apply(
        lambda x: str(x).replace("Neuquén ", "Neuquén")
    )
